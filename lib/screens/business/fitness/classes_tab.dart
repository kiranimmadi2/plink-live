    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Add fitness classes to your schedule',
  ...
  Future<void> _deleteClass(FitnessClassModel fitnessClass) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${fitnessClass.name}"?'),
        actions: [
          TextButton(
...
class _ClassCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final categoryColor = FitnessClassCategories.getColor(fitnessClass.category);
...
class _ClassFormSheetState extends State<_ClassFormSheet> {
  ...
  void initState() {
    ...
    _priceController =
        TextEditingController(text: c?.pricePerClass.toStringAsFixed(2) ?? '');
    _dropInPriceController =
        TextEditingController(text: c?.dropInPrice?.toStringAsFixed(2) ?? '');
    _durationController =
        TextEditingController(text: c?.durationMins.toString() ?? '60');
    _maxParticipantsController =
        TextEditingController(text: c?.maxParticipants.toString() ?? '20');
    _instructorController = TextEditingController(text: c?.instructor ?? '');
    _locationController = TextEditingController(text: c?.location ?? '');
    _caloriesController =
        TextEditingController(text: c?.caloriesBurned.toString() ?? '');
