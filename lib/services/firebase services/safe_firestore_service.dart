...
import '../../res/utils/firestore_error_handler.dart';
import '../../res/utils/network_utils.dart';
...
class SafeFirestoreService {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> watchMessages(String userId) {
    return _firestore
        ...
        .handleError((error) {
          ...
          debugPrint(
            'Stream error: ${FirestoreErrorHandler.getErrorMessage(errorType)}',
          );
  ...
  Future<void> batchUpdate(List<Map<String, dynamic>> documents) async {
    ...
    await FirestoreErrorHandler.handleFirestoreOperation(
      ...
      onError: (errorType, _) {
        debugPrint(
          'Batch update failed: ${FirestoreErrorHandler.getErrorMessage(errorType)}',
        );
      },
    );
  }
